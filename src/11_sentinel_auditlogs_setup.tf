...
# send pagopa.it organization log to google project organization
...
# send google project organization log to pub/sub
resource "google_logging_project_sink" "sentinel-project-pubsub-sink" {
  count            = var.organization_id == "" ? 0 : 1
...
resource "google_project_iam_binding" "log-writer-sentinel-organization-sink" {
  ...
  members = [
    google_logging_project_sink.sentinel-project-pubsub-sink[0].writer_identity
  ]
}
